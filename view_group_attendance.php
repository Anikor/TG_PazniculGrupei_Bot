...
$me = tg_require_auth();
tg_require_role($me, ['admin', 'monitor']);
$group_id = tg_resolve_group_id($me, $_GET['group_id'] ?? 0);
$today = date('Y-m-d');
$weekStart = date('Y-m-d', strtotime('monday this week'));
$monthStart = date('Y-m-01');
$periods = ['Today' => [$today, $today], 'This Week' => [$weekStart, $today], 'This Month' => [$monthStart, $today], 'All Time' => ['1970-01-01', $today],];
$stuQ = $pdo->prepare("SELECT id, name, tg_id FROM users WHERE group_id = ? ORDER BY name");
...
$statQ = $pdo->prepare("SELECT a.user_id,
  ...
  GROUP BY a.user_id");
$statQ->execute([':today_from1' => $today, ':today_from2' => $today, ':week_from1' => $weekStart, ':week_from2' => $weekStart, ':month_from1' => $monthStart, ':month_from2' => $monthStart, ':group_id' => $group_id, ':today_to' => $today,]);
$rowsByUser = [];
foreach ($statQ->fetchAll(PDO::FETCH_ASSOC) as $r) { $rowsByUser[(int)$r['user_id']] = $r; }
$periodKeyMap = ['Today' => ['today_t', 'today_a'], 'This Week' => ['week_t', 'week_a'], 'This Month' => ['month_t', 'month_a'], 'All Time' => ['all_t', 'all_a']];
$stats = [];
foreach ($students as $stu) {
    $sid = (int)$stu['id'];
    $r = $rowsByUser[$sid] ?? null;
    foreach ($periodKeyMap as $lbl => $keys) {
        [$tKey, $aKey] = $keys;
        $tot = $r ? (int)$r[$tKey] : 0;
        $abs = $r ? (int)$r[$aKey] : 0;
        $stats[$sid][$lbl] = ['total' => $tot, 'absent' => $abs, 'rate' => $tot ? round(100 * ($tot - $abs) / $tot, 2) : 0,];
    }
}
$sum = [];
foreach (array_keys($periods) as $lbl) { $sum[$lbl] = ['absent' => 0, 'total' => 0]; }
foreach ($students as $stu) {
    $sid = $stu['id'];
...
foreach ($sum as $lbl => $vals) {
    ...
    $summary[$lbl] = ['present' => $present, 'total' => $vals['total'], 'pct' => $pct,];
}
...
if ($gQ->execute([$group_id]) && ($n = $gQ->fetchColumn())) { $groupName = $n; }
?>
<!DOCTYPE html>
...
<div class="stat-cards"><?php foreach ($summary as $lbl => $d): ?>
<div class="stat"><h3><?= htmlspecialchars($lbl, ENT_QUOTES) ?></h3><p><?= "{$d['present']}/{$d['total']} ({$d['pct']}%)" ?></p></div><?php endforeach; ?>
</div>
<table><thead><tr><th></th><th>Student</th><?php foreach (array_keys($periods) as $lbl): ?>
<th><?= htmlspecialchars($lbl, ENT_QUOTES) ?></th><?php endforeach; ?>
...
<td><?= "{$r['absent']}/{$r['total']} (" . number_format($r['rate'], 2) . "%)" ?></td><?php endforeach; ?>
<td>
