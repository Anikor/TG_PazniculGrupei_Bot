...
$stats=[];foreach($students as $stu){$sid=$stu['id'];foreach($periods as $lbl=>[$f,$t]){[$tot,$abs]=fetchStats($pdo,$sid,$f,$t);$stats[$sid][$lbl]=['total'=>$tot,'absent'=>$abs,'rate'=>$tot?round(100*($tot-$abs)/$tot,2):0,];}}
