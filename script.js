...
window.addEventListener('pageshow', function (e) {
    ...
    if (e.persisted || (n && n.type === 'back_forward')) location.reload();
});
function onReady(fn) {
...
onReady(function () {
    ...
    function applyLayout(mode) {
        document.body.classList.toggle('layout-big', mode === 'big');
    ...
    const initial = document.body.classList.contains('layout-big') ? 'big' : 'small';
    setUI(initial);
...
onReady(function () {
    if (!document.getElementById('tg-bootstrap')) {
        try {
            sessionStorage.removeItem('tgBootTries');
        } catch {}
        return;
    }
    const tg = window.Telegram && window.Telegram.WebApp;
    ...
    if (!initData) { fail('Please open this app from Telegram.'); return; }
    let tries = 0;
    try {
        tries = parseInt(sessionStorage.getItem('tgBootTries') || '0', 10) || 0;
    } catch {}
    if (tries >= 3) {
        try {
            sessionStorage.removeItem('tgBootTries');
        } catch {}
        fail('Could not start a session. Fully close the Mini App and open it again.');
        return;
    }
    try {
        sessionStorage.setItem('tgBootTries', String(tries + 1));
    } catch {}
    fetch(location.href, {
        method: 'POST',
        ...
        .then(async r => {
            ...
            if (r.ok && j && j.success) {
                ...
                fail('Login failed: ' + ((j && j.error) || ('HTTP ' + r.status)));
            }
        })
        .catch(() => { fail('Network error while sending Telegram login.'); });
});
onReady(function () {
    ...
    submitBtn.addEventListener('click', async () => {
        ...
        try {
            ...
            try {
                ...
                j = m ? JSON.parse(m[0]) : JSON.parse(text);
            } catch (_) {}
            ...
            newBtn.addEventListener('click', () => {
                let href = `edit_attendance.php?offset=${encodeURIComponent(offset)}`;
            ...
            document.querySelectorAll('.att-toggle,.mot-toggle,.motiv-text').forEach(el => {
                el.disabled = true;
            });
        } catch (err) {
            console.error(err);
...
onReady(function () {
    ...
    if (primary) {
        primary.addEventListener('click', () => { location.href = 'greeting.php?view=primary'; });
    }
    const reset = document.getElementById('btnResetView');
    if (reset) {
        reset.addEventListener('click', () => { location.href = 'greeting.php?view=reset'; });
    }
});
onReady(function () {
    ...
    sel.addEventListener('change', () => {
        const url = new URL(location.href);
        url.searchParams.set('group_id', sel.value);
        location.href = url.toString();
    });
});
