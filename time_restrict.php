...
// Grace period and fallback cutoff come from config.php (env-overridable);
// defined() fallbacks keep this file usable standalone in tests.
function tr_default_tz(): DateTimeZone {
    return new DateTimeZone(defined('APP_TZ') ? APP_TZ : 'Europe/Chisinau');
}

function tr_grace_min(): int { return defined('MODERATOR_GRACE_MIN') ? (int)MODERATOR_GRACE_MIN : 20; }

function tr_fallback_cutoff(): string {
    return defined('MODERATOR_FALLBACK_CUTOFF') ? (string)MODERATOR_FALLBACK_CUTOFF : '18:00';
}
...
function moderator_cutoff_from_schedule(array $schedule, DateTimeInterface $targetDate, ?DateTimeZone $tz = null): ?DateTimeImmutable {
    ...
    if (!$maxEndTs) return null;
    return $maxEndTs->modify('+' . tr_grace_min() . ' minutes');
}
...
function can_user_edit_for_date(string $role, DateTimeInterface $targetDate, ?DateTimeZone $tz = null, array $schedule = []): array {
    ...
    if ($role === 'moderator') {
        ...
        if (!$cutoff) {
            $cutoff = DateTimeImmutable::createFromFormat('Y-m-d H:i', $now->format('Y-m-d') . ' ' . tr_fallback_cutoff(), $tz);
        }
        $ok = $now <= $cutoff;
        return [$ok, $ok ? '' : 'The moderator window is closed (after last lesson + ' . tr_grace_min() . ' min).'];
    }
    if ($role === 'monitor') {
