...
$target = $me;
if (!empty($_GET['tg_id'])) {
    $t = getUserByTgId(preg_replace('/\D/', '', (string)$_GET['tg_id']));
    if ($t) $target = $t;
}
if (!tg_can_view_user($me, $target)) tg_deny('Not allowed to view this student');
$user = $target;
$user_id = (int)$target['id'];
$tg_id = (string)$target['tg_id'];
...
function countClassDays($pdo, $uid, $from, $to) {
    $q = $pdo->prepare("SELECT COUNT(DISTINCT a.date) FROM attendance a WHERE a.user_id=:uid AND a.date BETWEEN :from AND :to AND a.present=1");
...
$rowsFor = function (string $from) use ($absRows) {
    return array_values(array_filter($absRows, fn($r) => $r['date'] >= $from));
};
$mk = function ($t, $a, $m, array $rows) {
    $t = (int)$t;
    $a = (int)$a;
    $m = (int)$m;
    return ['total' => $t, 'absent' => $a, 'motivated' => $m, 'unmotiv' => $a - $m, 'rows' => $rows];
};
...
$subjRows = $pdo->prepare("SELECT s.subject,s.type,COUNT(*) AS total,SUM(a.present=0) AS absent FROM attendance a JOIN schedule s ON s.id=a.schedule_id WHERE a.user_id=:uid GROUP BY s.subject,s.type");
$subjRows->execute([':uid' => $user_id]);
$subjStats = [];
while ($r = $subjRows->fetch(PDO::FETCH_ASSOC)) {
    $sub = $r['subject'];
    $typ = $r['type'] ?? '';
    $subjStats[$sub]['labels'][$typ] = ['total' => (int)$r['total'], 'absent' => (int)$r['absent']];
    if (!isset($subjStats[$sub]['overall'])) { $subjStats[$sub]['overall'] = ['total' => 0, 'absent' => 0]; }
...
$labMissCount = 0;
foreach ($subjStats as $d) { $labMissCount += (int)($d['labels']['lab']['absent'] ?? 0); }
...
if ($limitDT >= new DateTime($termStart)) {
    ...
    $byMonth = [];
    while ($r = $msQ->fetch(PDO::FETCH_ASSOC)) {
        $byMonth[$r['ym']] = ['total' => (int)$r['t'], 'absent' => (int)($r['a'] ?? 0)];
    }
    ...
    while ($cur <= $limitDT) {
        ...
        $monthStats = $byMonth[$cur->format('Y-m')] ?? ['total' => 0, 'absent' => 0];
...
$eggMessages = ["Flawless month! 100% attendance unlocked. 🏆", "No absences detected. The attendance gods are pleased. 😇", "Perfect streak achieved—keep it rolling! 🔥", "Achievement: „Never Miss a Beat”. 💯", "Legendary consistency—respect! 🙌", "You made the ‘Absent’ column feel lonely. 😅", "Model student vibes detected. 📚✨", "Attendance on point. Coffee well earned. ☕💪"];
...
if ($perfectTermToDate) {
    ...
    $showEgg = true;
    if ($afterWeek) { $eggTitle = '✅ Perfect Attendance'; $eggBody = $eggMsgExtra; }
    else { $eggTitle = '✅ Perfect First Month'; $eggBody = $eggMsg1[array_rand($eggMsg1)]; }
}
...
?>
<!doctype html>
<html class="<?= $theme === 'dark' ? 'dark-theme' : '' ?>" lang="en"><head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width,initial-scale=1">
<title>My Attendance</title>
<link rel="stylesheet" href="<?= asset('style.css') ?>"></head>
<body><br><div id="theme-switch"><label class="switch"><input id="theme-toggle" type="checkbox" <?= $theme === 'dark' ? 'checked' : '' ?>><span class="slider"></span></label><span id="theme-label"><?= $theme === 'dark' ? 'Dark' : 'Light' ?></span></div><br><br><button class="btn-nav" onclick='location.href="<?= htmlspecialchars($backUrl, ENT_QUOTES) ?>"'>← Back</button><h1>My Attendance</h1><?php if ($showEgg): ?>
<div class="card card--perfect" style="margin:12px 0;"><h3><?= $eggTitle ?></h3><p><?= htmlspecialchars($eggBody, ENT_QUOTES) ?></p><p style="opacity:.8"><small>Term window: <?= htmlspecialchars($termStart, ENT_QUOTES) ?> — <?= htmlspecialchars($termSpanEnd, ENT_QUOTES) ?></small></p><p style="opacity:.9"><small>Days this term to date: <?= $daysCalendar ?> (class days: <?= $daysClass ?>)</small></p><?php if (!$perfectTermToDate && $streakMonths > 0): ?>
<p style="opacity:.9"><small>Current streak: <?= $streakMonths ?> month<?= $streakMonths > 1 ? 's' : '' ?></small></p><?php endif; ?>
</div><?php endif; ?>
<div class="cards"><?php foreach ($stats as $label => $st): $rate = $st['total'] ? round(100 * $st['absent'] / $st['total'], 1) : 0; ?>
<div class="card"><h3><?= $label ?></h3><p><strong>Sessions:</strong> <?= $st['total'] ?></p><p><strong>Absent:</strong> <?= $st['absent'] ?></p><p style="margin-left:12px">– unmotivated: <?= $st['unmotiv'] ?></p><p style="margin-left:12px">– motivated: <?= $st['motivated'] ?></p><p><strong>Absence Rate:</strong> <?= $rate ?>%</p><?php if ($label === 'All Time'): ?>
<hr><p><strong>Lab Misses:</strong> <?= $labMissCount ?></p><p><strong>Est. Fee:</strong> <?= $labFee ?> Lei</p><?php endif; ?>
</div><?php endforeach; ?>
</div><h2>By Subject Absence Rates</h2><table class="subj-table"><thead><tr><th>Subject</th><th>Curs Rate</th><th>Sem Rate</th><th>Lab Rate</th><th>Overall Rate</th></tr></thead><tbody><?php
foreach ($subjStats as $sub => $data):
$oTotal = $data['overall']['total'];
$oAbsent = $data['overall']['absent'];
$oRate = $oTotal ? 100 * $oAbsent / $oTotal : 0;
?>
<tr><td><?= htmlspecialchars($sub, ENT_QUOTES) ?></td><?php
foreach (['curs', 'sem', 'lab'] as $t):
$dTotal = $data['labels'][$t]['total'] ?? 0;
$dAbsent = $data['labels'][$t]['absent'] ?? 0;
$dRate = $dTotal ? (100 * $dAbsent / $dTotal) : 0;
?>
<td><?= $dAbsent ?>/<?= $dTotal ?> (<?= number_format($dRate, 2) ?>%)</td><?php endforeach; ?>
<td><?= $oAbsent ?>/<?= $oTotal ?> (<?= number_format($oRate, 2) ?>%)</td></tr><?php endforeach; ?>
</tbody></table><?php foreach ($stats as $label => $st): if (empty($st['rows'])) continue; ?>
<h2><?= $label ?> Absences</h2><table><thead><tr><th>Date</th><th>Time</th><th>Subject</th><th>Type</th><th>Reason</th></tr></thead><tbody><?php foreach ($st['rows'] as $r): ?>
<tr><td><?= htmlspecialchars($r['date'], ENT_QUOTES) ?></td><td><?= htmlspecialchars($r['time_slot'], ENT_QUOTES) ?></td><td><?= htmlspecialchars($r['subject'], ENT_QUOTES) ?></td><td><?= htmlspecialchars($r['type'], ENT_QUOTES) ?></td><td><?= $r['motivation'] ? htmlspecialchars($r['motivation'], ENT_QUOTES) : '<em>none</em>' ?></td></tr><?php endforeach; ?>
</tbody></table><?php endforeach; ?>
<script src="<?= asset('script.js') ?>"></script></body></html>
